...
def format_http_request(raw_req: str) -> str:
    ...
    # Scan all lines to detect multipart (headers may be double-spaced so we can't rely on split)
    is_multipart = any(re.match(r'(?i)content-type:\s*multipart/form-data', line) for line in lines)

    if is_multipart:
        # Body starts at the first boundary line (starts with '--' followed by non-space)
        body_start_idx = None
        ...
        if body_start_idx is not None:
            # Collapse blank lines from the header section only
            header_lines = [line for line in lines[:body_start_idx] if line.strip()]
            headers_str = '\n'.join(header_lines)
            headers_str = re.sub(r"HTTP/[23]\.?0?", "HTTP/1.1", headers_str, count=1)
            headers_str = re.sub(r"(?i)Connection:\s*keep-alive", "Connection: close", headers_str)

            body_str = '\n'.join(lines[body_start_idx:])
            body_length = len(body_str.encode('utf-8'))
            if re.search(r"(?i)Content-Length:", headers_str):
                headers_str = re.sub(r"(?i)Content-Length:\s*\d+", f"Content-Length: {body_length}", headers_str)

            logger.debug("Formatted multipart/form-data request")
            return headers_str + '\n\n' + body_str
...
def format_history_text(raw_text: str) -> str:
    ...
    # Scan all lines for multipart (headers may be double-spaced)
    is_multipart = any(re.match(r'(?i)content-type:\s*multipart/form-data', line) for line in lines)
    ...
    if is_multipart:
        ...
        if body_start_idx is not None:
            header_lines = [line for line in lines[:body_start_idx] if line.strip()]
            body_str = '\n'.join(lines[body_start_idx:])
            logger.debug("Formatted multipart history text")
            return '\n'.join(header_lines) + '\n\n' + body_str
        return req.strip()
