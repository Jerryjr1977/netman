...
def build_ai_tab(notebook):
    ...
    # --- Agent Goal Frame ---
    goal_frame = tk.Frame(ai_tab)
    goal_frame.pack(fill=tk.X, padx=20, pady=(0, 5))

    tk.Label(goal_frame, text="Agent Goal:", font=("Arial", 10, "bold")).pack(side=tk.LEFT)

    agent_goal_var = tk.StringVar()
    goal_entry = tk.Entry(goal_frame, textvariable=agent_goal_var, width=60)
    goal_entry.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)

    def _set_agent_goal():
        import __main__ as gui_main
        goal_text = agent_goal_var.get().strip()
        if not goal_text:
            return
        # Resolve the singleton agent from the main GUI module
        _agent = getattr(gui_main, 'agent', None) or agent_engine.NetManAgent()
        _agent.set_goal(goal_text)
        if ai_output:
            ai_output.insert(tk.END, f"[AGENT GOAL SET]: {goal_text}\n\n")
            ai_output.see(tk.END)

    goal_entry.bind("<Return>", lambda e: _set_agent_goal())
    tk.Button(goal_frame, text="Set Goal", bg="darkcyan", fg="white",
              command=_set_agent_goal).pack(side=tk.LEFT, padx=5)
