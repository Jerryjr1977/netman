...
def _get_ca_key_cert():
    ...
    if _ca_key_cache is None:
        with open(_project_path("jerry_ca.key"), "rb") as f:
            _ca_key_cache = serialization.load_pem_private_key(f.read(), password=None)
        with open(_project_path("jerry_ca.pem"), "rb") as f:
...
def load_task_json():
    ...
    task_json_path = _project_path("task.json")
...
def clean_stale_certs(days=CERT_MAX_AGE_DAYS):
    ...
    cert_patterns = ["*.crt", "*.key", "*.chain.pem"]
    cert_files = []
    for pattern in cert_patterns:
        cert_files.extend(glob.glob(_project_path(pattern)))
    for cert_file in cert_files:
        if os.path.basename(cert_file) in ["jerry_ca.pem", "jerry_ca.key"]:
            continue
        try:
...
def forge_cert(domain):
    ...
    cert_file = _project_path(f"{domain}.crt")
    key_file = _project_path(f"{domain}.key")
    chain_file = _project_path(f"{domain}.chain.pem")
...
def handle_client(client_socket):
    ...
    def maybe_log_traffic(target_host, raw_request):
        ...
        log_file_path = _project_path("http_history.log")
        def _write():
            try:
    ...
    def forward_request(client_sock, target_host, target_port, request_bytes, use_tls=False):
    ...
    def handle_request(client_sock, first_line, header_lines, body_bytes, is_tls=False, connect_host=None, connect_port=None):
        ...
        maybe_log_traffic(target_host, request_bytes)
...
def start_proxy(port=8080, global_listen=False):
    global is_running
    if not os.path.exists(_project_path("jerry_ca.pem")) or not os.path.exists(_project_path("jerry_ca.key")):
